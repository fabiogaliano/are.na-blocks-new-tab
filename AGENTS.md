...
There is no bundler; load the unpacked `extension/` directory directly in Chrome or Firefox (`chrome://extensions` → Load unpacked, or `about:debugging` → Load Temporary Add-on). Run `bun run test` for the Vitest suite and `bun run check` to catch syntax regressions. Use `web-ext run --source-dir extension` if you have Mozilla’s tooling installed for faster iteration, though it remains optional.
